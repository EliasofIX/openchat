...
- **In-flight streams:** `use-chat` bumps a generation counter on `setMessages` (conversation switch / load) and aborts the fetch; flush/finish/persist callbacks must no-op when their captured generation is stale — otherwise a reply started in chat A can be saved into chat B. `chat.tsx` must not call `setMessages` (the aborting `setAll`) when `upsertActive` assigns `activeId` mid-stream on the first persist of a new chat (`lastLoadedId === null` → new id) — only sync `lastLoadedId`; deletions still load via `setAll`. Sidebar `selectConversation` must no-op when the clicked id is already active (mobile overlay dismiss re-clicks abort streams if it reloads).
...
- **Model context windows:** read `context_length` from OpenRouter `/models` and `*.context_length` from Ollama `/api/show` `model_info`; cap Ollama by parsed `num_ctx`. Extend `ModelCapabilities` in `src/lib/model-capabilities.ts` — do not add a separate metadata route. OpenRouter `tools` comes from `supported_parameters` (not assumed true); default `tools: false` until capabilities resolve. Attach tools via client `enabledTools: ("save_memory" | "web_search")[]` — only when the matching setting is on **and** `!loading && capabilities.tools`.
- **Client never sees API keys** — keys come from env or user settings, resolved server-side
- **Upstream chat errors:** `ai-client.stream()` awaits the HTTP response before returning the SSE generator so 401/404/tool-routing failures become a normal `Response` (via `UpstreamHttpError`), not Next's opaque "failed to pipe response".
...
- **OpenRouter ZDR:** `UserSettings.zdrOnly` (Model providers → OpenRouter) injects `provider: { zdr: true }` on every OpenRouter inference path — chat stream, title completion, and TTS — via `openRouterZdrProvider`. Skip capabilities/metadata fetches.
- **OpenRouter prompt caching:** OpenRouter-only (`src/lib/prompt-cache.ts`, `src/lib/system-prompt.ts`). Pass `session_id` as `{conversationId}:{model}` via `buildCacheSessionId` (plus `x-session-id` header) for sticky routing — omit when `activeId` is null so new chats never send `"null:<model>"`. Keep the system message stable — memories go in a separate injected `user` message (`memoryContext`), not trailing system content (required for Gemini). Freeze `systemPrompt` + `memoryContext` for the duration of a `send()` tool-round loop so mid-turn memory saves don't bust the prefix. Do **not** send assistant `reasoning` upstream — UI-only. Anthropic models get top-level `cache_control`; explicit mode adds per-block breakpoints on large attachments only (auto mode reserves explicit slots for attachments). Resolve mode via `resolvePromptCachingMode(model, cachingMode)` — treat client `"none"` (capabilities still loading / default) as unknown and fall back to the model id, do not use `cachingMode ?? …` or an explicit `"none"` disables caching. Request `stream_options: { include_usage: true }` and emit a trailing NDJSON `{ "p": "usage", "prompt", "cached", "written" }` line even on client abort when usage was received — call `emitCacheUsage()` *before* setting `streamClosed = true` (it no-ops once closed) — switch to NDJSON whenever caching is on. Settings live in `UserSettings.promptCaching` (enabled + TTL).
...
- **Markdown code blocks:** react-markdown v10 dropped `code`'s `inline` prop — detect fences via the `pre` wrapper and render `CodeBlock` there. Never return a `<div>` from `code` (it nests inside `<p>` for backticks and causes hydration errors).
...
**Intentionally missing** (add only when asked): auth, multi-user DB, tool calling, rate limiting, analytics, i18n.

Do not add a heavy test framework. Small `node:test` suites for non-trivial pure helpers (`npm test`) are fine when they lock a real contract.
...
| Agent memory | `src/hooks/use-memories.ts`, `src/lib/memory-tools.ts`, `src/lib/system-prompt.ts`, `src/components/chat/memory-settings.tsx` — localStorage `openchat:memories`; stable system via `buildStableSystemPrompt`, memories as injected `user` (`buildMemoryContextContent`) only when `settings.memory.enabled` (independent of tools); main model gets `save_memory` when enabled **and** `ModelCapabilities.tools`; client executes tool, follow-up round in `use-chat.ts`. Check `saveMemories` return value — never report saved on quota failure. Single eviction policy in `storage.ts` (`capMemories`: agent entries evicted before user at cap). Gate send on memory/settings hydration; listen for `storage` events on `openchat:memories` for multi-tab sync. |
| Web search | `src/lib/web-search.ts`, `src/app/api/search/route.ts`, settings `webSearch.enabled` (default off). Client tool loop executes `web_search` by POSTing `/api/search` (browser cannot scrape DDG — CORS). Proxy fetches DuckDuckGo HTML, falls back to Instant Answer JSON; no commercial SERP SDKs/keys. Persist `Message.sources`; cite `[n]` via `linkifyCitationMarkers` + sources panel. |
...
| Read aloud (Grok TTS) | `src/hooks/use-message-tts.ts`, `src/components/chat/tts-now-playing.tsx`, `src/app/api/tts/route.ts`, `src/lib/tts-cache.ts` — shared player with abort + generation guards; LRU cache (memory + IndexedDB) by voice+ZDR+text with entry/byte caps; `/api/tts` enforces 15k chars and forwards `req.signal`; surface failures in the now-playing error pill; voice in `UserSettings.tts` |
...
| Brand / favicon / app icon | `assets/brand/icon.svg` (master) → `src/app/icon.svg`, `src/app/favicon.ico`, `src/app/apple-icon.png`, `assets/brand/icon.png`; Electron via `package.json` `build.icon` |
