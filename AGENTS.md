...
- Battery: keep `backgroundThrottling: true` and push a `power-mode` IPC signal (on-battery / hidden / minimized / blurred) that `electron/preload.js` turns into a `.low-power` class on `<html>`; gate GPU-heavy effects (glass `backdrop-filter`) behind it. Renderer code may read that class to throttle JS too — `use-chat` coalesces stream re-renders from one-per-animation-frame (~60fps) to ~10fps in low-power, since Chromium only auto-throttles RAF when *hidden*, not when blurred-but-visible. Avoid `powerSaveBlocker` for idle UI and any polling that wakes the CPU.
