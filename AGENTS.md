...
| Read aloud (Grok TTS) | `src/hooks/use-message-tts.ts`, `src/components/chat/tts-now-playing.tsx`, `src/app/api/tts/route.ts`, `src/lib/tts-cache.ts` — shared player with abort + generation guards; LRU-cache audio by voice+text so replays skip the network; voice in `UserSettings.tts` |
